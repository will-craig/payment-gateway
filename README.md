...
I've taken the liberty of assuming the 'declined' status as the default one, in this scenario. I would've liked to return a specific response for this scenario, 
...
I've skipped controller level unit tests as all we would be testing here are mocks, and the code coverage should be handled by our integration level tests.
...
The integration tests cover the end2end flow of the payment processing. I included integration scenarios for the main merchant-visible outcomes:
  - authorised
  - declined
  - rejected (validation failure)
  - get existing payment / not found

One thing worth noting is I wouldn't ordinarily create integration tests which rely on a third party (the bank simulator in this case), being a dependency without a conditional stub that can be configured to sit in its place when offline.
The simulator is a special case as it is essentially a stub itself, and is designed to be run locally. And I wanted to avoid doing too many things not specified the requirements.

## Reflection/ What I would do differently next time / Suggestions for improvement

- With regard to the validation, I created the validation rules as static functions for swiftness and ease of use. What would have been nice, would be to recreate them (non-statically) with a common interface.
This would have allowed me to mock them in the Validator's unit tests. Due to this lack of isolation, the unit tests for the validator are more like integration tests, as they are testing the actual validation rules, rather than just the orchestration of them. 
A common interface would've also allowed me to do something fancy like use the strategy pattern and preload the rules into a list that can be iterated over.
Ultimately I went for simplicity as I want to avoid over engineering, and the current implementation is straightforward and easy to understand.

- Another area I would improve is the error handling for the bank client. As mentioned it mostly functions as a wrapper for the HttpClient, but I would like to implement some retry logic for transient errors
and also some better handling for different response codes. For example, if the bank is down, it would be nice to have a specific response for this, rather than what i've opted for, of just a generic "declined" status.

- One of my regrets is number of dtos and dto mappers being used through the solution. A symptom of using this layered architecture; it's mostly unavoidable. But with more time I would've liked to find a way of reducing the number/ reusing common models ect..

- Maybe some sort of idempotency key for the payment processing. Did this exact payment and card just get submitted? Maybe next time..
